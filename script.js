...
async function loadGuestList() {
    try {
        ...
        })).filter(guest => guest.fullName); // Filter out entries with no name
        
        console.log('Loaded guest list:', guestList); // Debug log
    } catch (error) {
...
function checkInvitation() {
    ...
    // Clean the input for phone number comparison (remove non-digits)
    ...
    const guest = guestList.find(g => {
        ...
        // Check phone number match - only if both input and guest phone are not empty
...
function initializeCountdown() {
    // This countdown points to the White Wedding on the 27th
...
function setupTimeConversion() {
    ...
    timeDisplays.forEach(display => {
        ...
        // Extract the date from the sibling .event-date element
        const dateString = parentEventCard.querySelector('.event-date').textContent.trim().replace(/,/g, '');
        const dateParts = dateString.split(' ');
        const eventDate = `${dateParts[2]} ${dateParts[1]}, ${dateParts[3]}`; // Format as "Month Day, Year"
...
function setupMusicPlayer() {
    ...
    audio.addEventListener('error', () => {
        console.warn('Failed to load audio file, trying next song');
        playBackgroundMusic();
    });
}

function handleMusicEnded() {
    // Small delay to ensure the current song is properly finished
    setTimeout(() => {
        playBackgroundMusic();
    }, 100);
}
...
function playBackgroundMusic() {
    ...
    if (musicPlaylist.length === 1) {
        ...
        // Select a different random song
        let randomIndex;
        do {
    ...
    // Add a small delay before playing to ensure the src is set
    setTimeout(() => {
