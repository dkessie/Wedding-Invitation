...
function checkInvitation() {
    ...
    const guest = guestList.find(g => 
        g.fullName === input || 
        g.phoneNumber === input || 
        g.phoneNumber === input.replace(/\D/g, '')
    );
...
function initializeCountdown() {
    const weddingDate = new Date('September 27, 2025 08:00:00 GMT');
...
function setupTimeConversion() {
    ...
    timeDisplays.forEach(display => {
        ...
        const localTimeElement = display.parentElement.nextElementSibling;
        ...
        if (localTimeElement && localTimeElement.classList.contains('local-time')) {
            ...
            const weddingDate = new Date('2025-09-27T' + hour.toString().padStart(2, '0') + ':' + minutes + ':00.000Z');
            const localTime = weddingDate.toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit', hour12: true, timeZoneName: 'short' });
...
function setupMusicPlayer() {
    ...
    audio.addEventListener('ended', playBackgroundMusic);
}
...
function playBackgroundMusic() {
    ...
    } while (musicPlaylist.length > 1 && randomIndex === lastPlayedIndex);
